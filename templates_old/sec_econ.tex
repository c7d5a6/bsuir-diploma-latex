...
\par
\begin{tabular}{@{}ll@{ --- }p{0.74\textwidth}}
где & $ \text{А}^\text{о}_i $ & амортизационные отчисления, \byr. \\[\parsep]
\end{tabular}
...
где & $ \text{Р}^\text{п}_i $ & переменные затраты на производство и реализацию, \byr; \\
    ...
    & $ \text{Р}^\text{н}_i $ & налоги, уплачиваемые из прибыли. \\[\parsep]
\end{tabular}
...
где & $ r $ & реальная (без учета компенсации за инфляцию) безрисковая ставка ссудного процента, \%; \\
    ...
    & $ g_i $ & премия за отдельный риск по конкретному фактору, \%. \\[\parsep]
\end{tabular}
...
\par
\begin{tabular}{@{}ll@{ --- }p{0.74\textwidth}}
где & $ t $ & номер отрезка времени расчетного периода с момента старта проекта, \%. \\[\parsep]
\end{tabular}
